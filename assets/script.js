...
$('body').on('click', '#percentageCalculator', function() {
    $('.lossCalculator,.averageCalculator,#percentageCalculator').hide();
    $('.percentageCalculator,#averageCalculator,#lossCalculator').show();
});
$('body').on('click', '#lossCalculator', function() {
    $('.percentageCalculator,.averageCalculator,#lossCalculator').hide();
...
$('body').on('click', '.addmore', function(event) {
	...
	newField.find('.avg_buy_price').attr('placeholder','Buy Price '+countr);
	newField.find('.avg_qty').attr('placeholder','Stock Quantity '+countr);
	if(!newField.find('.avg_close').length){
		newField.prepend('<div style="height: 25px;width: 100%;"><div class="avg_close">[<i class="fa-solid fa-xmark"></i>]</button></div>');
	}
